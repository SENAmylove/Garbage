        ...
        with open(_count_file, 'r') as freader:
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_1 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_1:
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_2 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_2:
                        wfile.write(_count_frame)
            wfile.close()
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_3 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_3:
                        wfile.write(_count_frame)
            wfile.close()
...
def split_dataset_with_overlap_by_1_equal(file_path: list):
    for _count_file in file_path:
        with open(_count_file, 'r') as freader:
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_4 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_4:
                        wfile.write(_count_frame)
            wfile.close()
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_5 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_5:
                        wfile.write(_count_frame)
            wfile.close()
            ...
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_6 <= int(_count_frame.split(',')[3]) and int(
                            _count_frame.split(',')[3]) <= end_timestamp_6:
                        wfile.write(_count_frame)
            wfile.close()
...
def split_dataset_with_no_overlap(file_path: list):
    for _count_file in file_path:
        with open(_count_file, 'r') as freader:
            ...
            # {object_id : { frame_id : content line }}
            _count_file_path_to_be_saved = os.path.join(os.path.dirname(_count_file)
                                                        , os.path.basename(_count_file).split('.')[
                                                            0] + '_0-3mins_no_overlap.csv')
            wfile = open(_count_file_path_to_be_saved, 'w')
            for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                for _count_frame in _count_object_item:
                    if start_timestamp_1 <= int(_count_frame.split(',')[3]) and int(
    ...
    for _count_file in file_path:
        with open(_count_file, 'r') as freader:
            ...
            for _count in range(0, 15):
                timestamps.append(
                    [start_timestamp + 1000 * 60 * 1 * _count, start_timestamp + 1000 * 60 * 1 * (_count + 1)])
            ...
            for index, (start, end) in enumerate(timestamps):
                ...
                _count_file_path_to_be_saved = os.path.join(os.path.dirname(_count_file)
                                                            , os.path.basename(_count_file).split('.')[
                                                                0] + f'_{index}-{index + 1}mins_no_overlap.csv')
                wfile = open(_count_file_path_to_be_saved, 'w')
                for _count_object_id, _count_object_item in content_dict_with_object_id.items():
                    for _count_frame in _count_object_item:
...
def insert_gaussian_noise(file_path: str):
    ...
    for _object_id, _object_item in content_dict_with_object.items():
        ...
        for _frame_index, _frame_id in enumerate(sorted_frame_id_list):
            ...
            _temp_averaged_list = [(_current_row[_col_index] + _next_row[_col_index]) / 2 for _col_index in
                                   range(len(_object_item[_frame_id]))]
    ...
    noised_file_name = os.path.splitext(file_path)[0] + '_noised_local_x_local_y.csv'
    sorted_result_list = []
    with open(noised_file_name, 'w') as writer:
        for _object_id, _object_item in content_dict_with_object.items():
            ...
            for _frame_index, _frame_id in enumerate(sorted_frame_id_list):
                ...
                sorted_result_list.append(_object_item[_frame_id])
    np.savetxt(noised_file_name,np.array(sorted_result_list),fmt='%d,%.1f,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%.6f,%.6f,%d,%d,%d,%.6f,%.6f')
